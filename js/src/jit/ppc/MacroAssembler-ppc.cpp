...
EmitBranchOnCondition(MacroAssemblerPPC& masm, Assembler::Condition c, Label* label)
{
    ...
    switch (c) {
      ...
      case Assembler::Signed:
        // After cmp, negative value sets CR_LT (not CR_SO which is XER overflow)
        bo = BO_TRUE; bi = CR_LT;
        break;
...
void MacroAssemblerPPCCompat::convertDoubleToInt32(FloatRegister src, Register dest, Label* fail, bool negativeZeroCheck)
{
    ...
    // If NOT overflow (not equal to INT_MIN), skip the fail branch
    ...
    if (negativeZeroCheck) {
        Label notZero;
        as_cmpwi(dest, 0);
        ma_b(&notZero, Assembler::NotEqual);
...
// GC barriers
// =====================================================================
// patchableCallPreBarrier: removed PPC override — uses shared template
// from MacroAssembler.h which correctly calls the preBarrier trampoline.
...
void MacroAssemblerPPCCompat::branchTestValue(Condition cond, const ValueOperand& lhs, const Value& rhs, Label* label)
{
    ...
    if (cond == NotEqual) {
        // Types differ -> values not equal -> branch
        Label typesEqual;
        ma_b(&typesEqual, Equal);
        ma_b(label);
        bind(&typesEqual);
        // Types equal, check payload
        ma_li(ScratchRegister, Imm32(rhs.toNunboxPayload()));
        ma_b(lhs.payloadReg(), ScratchRegister, label, NotEqual);
    } else {
        // Equal: types must match first
        Label done;
        ma_b(&done, NotEqual);
        // Types equal, check payload
        ma_li(ScratchRegister, Imm32(rhs.toNunboxPayload()));
        ma_b(lhs.payloadReg(), ScratchRegister, label, Equal);
        bind(&done);
    }
}
...
void
MacroAssemblerPPCCompat::ensureDouble(const Address& source, FloatRegister dest, Label* failure)
{
    Label isDouble, done;
    asMasm().branchTestDouble(Assembler::Equal, source, &isDouble);
    asMasm().branchTestInt32(Assembler::NotEqual, source, failure);
    // Big-endian: payload is at offset+4, not offset+0.
    // The shared template calls convertInt32ToDouble(source) which reads
    // from offset+0 (the type tag). We must read from the payload offset.
    convertInt32ToDouble(ToPayload(source), dest);
    jump(&done);
    bind(&isDouble);
    unboxDouble(source, dest);
    bind(&done);
                                           ...
                                           Register temp, Label* label)
{
    ...
    // PPC is big-endian NUNBOX32: TYPE at offset+0, PAYLOAD at offset+4.
    // Must load PAYLOAD (the object pointer), not TYPE.
    loadPtr(Address(address.base, address.offset + NUNBOX32_PAYLOAD_OFFSET), temp);
...
MacroAssembler::PushRegsInMask(LiveRegisterSet set)
{
    ...
    // Reserve and store GPRs first (closest to spillBase, at higher addresses).
    // Must use decrementing offset so backward iterator stores highest register
    // at highest address — matching the GC scanner's read order.
    reserveStack(diffG);
    ...
    // Reserve and store FPRs below GPRs.
    // PPC doesn't need the MIPS alignment dance — just reserve and store.
    reserveStack(diffF);
    int32_t offsetF = diffF;
    for (FloatRegisterBackwardIterator iter(set.fpus().reduceSetForPush()); iter.more(); ++iter) {
        offsetF -= sizeof(double);
        ma_sd(*iter, Address(StackPointer, offsetF));
    }
    MOZ_ASSERT(offsetF == 0);
}
...
MacroAssembler::PopRegsInMaskIgnore(LiveRegisterSet set, LiveRegisterSet ignore)
{
    ...
    // Restore FPRs first (they're at lower addresses, stored second).
    int32_t offsetF = diffF;
    for (FloatRegisterBackwardIterator iter(set.fpus().reduceSetForPush()); iter.more(); ++iter) {
        offsetF -= sizeof(double);
        if (!ignore.has(*iter))
            ma_ld(*iter, Address(StackPointer, offsetF));
    }
    freeStack(diffF);

    // Restore GPRs (at higher addresses, stored first).
    // Decrementing offset to match PushRegsInMask storage order.
    int32_t offsetG = diffG;
    for (GeneralRegisterBackwardIterator iter(set.gprs()); iter.more(); ++iter) {
        offsetG -= sizeof(intptr_t);
        if (!ignore.has(*iter))
            loadPtr(Address(StackPointer, offsetG), *iter);
    }
    freeStack(diffG);
}
