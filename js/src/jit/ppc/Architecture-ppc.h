...
class Registers
{
  public:
    ...
    static const uint32_t Allocatable = 15; // r15-r28 (14) + some volatile (r14 reserved for savedSP)
    ...
    static const SetType NonAllocatableMask =
        ...
        (1 << r12) | // scratch2
        (1 << r13) | // reserved (SDA)
        (1 << r14) | // savedSP from enterJIT — needed for bailout crash recovery
        (1 << r18) | // ICStubReg — must not be clobbered by regalloc
        (1 << r19) | // argsBase in VMWrapper — clobbered by every VM call
        (1 << r28) | // vp (result pointer) in EnterJIT — must survive Ion calls
    ...
    static const SetType JSCallMask = (1 << r5) | (1 << r4);  // must match JSReturnReg_Type=r5, JSReturnReg_Data=r4
