...
class MacroAssemblerPPCCompat : public MacroAssemblerPPC
{
  ...
  public:
    ...
    // GC barriers
    // =====================================================================
    void patchableCallPreBarrier(const Address& addr, MIRType type);
    ...
    void loadInt32OrDouble(Register base, Register index, FloatRegister dest, int32_t shift) {
        Label notInt, done;
        // Compute address of type tag
        BaseIndex typeAddr(base, index, TimesOne, NUNBOX32_TYPE_OFFSET);
        branch32(Assembler::NotEqual, typeAddr, ImmType(JSVAL_TYPE_INT32), &notInt);
        // Load int32 payload and convert
        BaseIndex payloadAddr(base, index, TimesOne, NUNBOX32_PAYLOAD_OFFSET);
        load32(payloadAddr, ScratchRegister);
        convertInt32ToDouble(ScratchRegister, dest);
        jump(&done);
        bind(&notInt);
        // Load as double
        BaseIndex valAddr(base, index, TimesOne, 0);
        loadDouble(valAddr, dest);
        bind(&done);
    }
